...
export const getScheduleConfig = () => {
  ...
  if (weeksAhead < SCHEDULE_CONFIG.MIN_SLOT_GENERATION_WEEKS) {
    throw new Error(`SLOT_GENERATION_WEEKS must be at least ${SCHEDULE_CONFIG.MIN_SLOT_GENERATION_WEEKS}`);
  }

  if (weeksAhead > SCHEDULE_CONFIG.MAX_SLOT_GENERATION_WEEKS) {
    throw new Error(`SLOT_GENERATION_WEEKS cannot exceed ${SCHEDULE_CONFIG.MAX_SLOT_GENERATION_WEEKS}`);
  }
